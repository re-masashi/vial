...
impl VM {
    ...
    pub fn from_slices(
        ...
        Self::new(
            ...
            struct_layouts.unwrap_or(&[]).to_vec(),
            enum_layouts.unwrap_or(&[]).to_vec(),
        )
    }
    ...
    fn execute_instruction(&mut self, instruction: &DecodedInstruction) -> Result<(), String> {
        match instruction.opcode {
            ...
            _ => {
                // For unimplemented opcodes, just skip them
                eprintln!("Warning: Unimplemented opcode {:?}", instruction.opcode);
            }
        }
