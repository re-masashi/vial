use crate::hir::{
    AllocationInfo, AllocationPreference, Builder, ConstantFolder, 
    ControlFlowGraph, DominatorTree, FunctionSignature, IntType, Opcode, 
    Param, Terminator, Type, TypeContext, ValueId, BasicBlock, BlockId, Function, 
    TypeId, PhiNode
};
...
fn test_ssa_builder_with_phi_nodes() {
    ...
    builder.add_phi_node(phi_result, i32_type, vec![
        (then_val, then_block),
        (else_val, else_block),
    ]);
